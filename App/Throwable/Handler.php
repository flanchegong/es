...
class Handler
{
    ...
    public static function shutDownHandler()
    {
        ...
        if (!empty($error)) {
            Logger::getInstance()->log(var_export($error, true), 4);
    ...
    {
        Logger::getInstance()->log(var_export(['description' => $description, 'file' => $file, 'line' => $line], true), 4);
        if (Config::getInstance()->getConf('app.dingtalk.enable')) {
    ...
    {
        Logger::getInstance()->log($throwable->__toString(), 4);
        if (Config::getInstance()->getConf('app.dingtalk.enable')) {
